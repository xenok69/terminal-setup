...
My personal Windows Terminal setup, a collection of config files that give you a clean, themed PowerShell experience with a system info splash screen. Includes a customized oh-my-posh prompt and a fastfetch greeting with ANSI art.

## What's Included

| file | description |
| ---- | ----------- |
| `bin/settings.json` | Windows Terminal config - Dimidium color scheme, FiraCode Nerd Font |
...
This sets up the **Dimidium** color scheme and **FiraCode Nerd Font Mono**.
...
```powershell
chafa -s 50x50 --symbols ascii --color-space din99d --fg-only "$($env:APPDATA)\fastfetch\logo.png" > "$($env:APPDATA)\fastfetch\logo.ans"
```
